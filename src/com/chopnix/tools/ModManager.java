...
import com.chopnix.dialogs.ModpackUpdateDialog;
...
public class ModManager extends JDialog {
	...
	private class ModManagerWorker extends SwingWorker<Boolean, Void> {
		...
		protected Boolean doInBackground() throws IOException,
			...
			if (!upToDate()) {
				...
				ModPack pack = ModPack.getPack(LaunchFrame
						.getSelectedModIndex());
				File modPackZip = new File(installPath + File.separator
						+ "temp" + File.separator + pack.getDir()
						+ File.separator + pack.getModFileName());
				if (modPackZip.exists()) {
					modPackZip.delete();
				}

				try {
					new File(installPath + "/temp/" + pack.getDir() + "/")
							.mkdir();
		...
		public void downloadUrl(String filename, String urlString)
			...
			try {
				...
				URL url = new URL(LaunchFrame.getFullLink(ModPack.getPack(
						LaunchFrame.getSelectedModIndex()).getUrl()));
				...
				while ((count = in.read(data, 0, 1024)) != -1) {
					...
					if (steps > 100) {
						...
						label.setText(decimalFormat.format(amount / 1048576)
								+ "MB / "
								+ decimalFormat.format(modPackSize / 1048576)
								+ "MB");
					}
				}
			} finally {
		...
		protected void downloadModPack(ModPack pack) throws IOException,
				NoSuchAlgorithmException {
			System.out.println("Downloading");
			String basePath = Settings.getSettings().getInstallPath()
					+ "/temp/" + pack.getDir() + "/";

			new File(basePath).mkdirs();
			new File(basePath + pack.getModFileName()).createNewFile();
			downloadUrl(basePath + pack.getModFileName(),
					LaunchFrame.getFullLink(pack.getUrl()));
		...
		private void cleanOldInstallation(ModPack pack) {
			...
			File baseFolder = new File(Settings.getSettings().getInstallPath()
					+ File.separator + pack.getDir());

			if (baseFolder.exists()) {
				ArrayList<Pattern> uninstallList = new ArrayList<Pattern>();
				if (uninstallList.size() > 0) {
					Logger.logInfo("Using to uninstall: " + pack.getDir()
							+ "/uninstall.lst");
					File cleanFile = new File(Settings.getSettings()
							.getInstallPath()
							+ "/temp/"
							+ pack.getDir()
							+ "/uninstall.lst");
					...
					File modsFolder = new File(baseFolder, File.separator
							+ "minecraft" + File.separator + "mods");
					if (modsFolder.exists()) {
						cleanModFolder(modsFolder, uninstallList);
					}
					File coreModsFile = new File(baseFolder, File.separator
							+ "minecraft" + File.separator + "coremods");

					if (coreModsFile.exists()) {
						cleanModFolder(coreModsFile, uninstallList);
					}

					File instModsFile = new File(baseFolder, File.separator
							+ "instMods");
					...
					File configFile = new File(baseFolder, File.separator
							+ "minecraft" + File.separator + "config");
					if (configFile.exists()) {
						cleanModFolder(instModsFile, uninstallList);
					}
				} else {
					try {
						FileUtils.delete(new File(baseFolder, File.separator
								+ "minecraft" + File.separator + "mods"));
						FileUtils.delete(new File(baseFolder, File.separator
								+ "minecraft" + File.separator + "coremods"));
						FileUtils.delete(new File(baseFolder, File.separator
						...
						if (!saveConfiguration) {
							FileUtils.delete(new File(baseFolder,
		...
		protected void installMods(String modPackName, String dir)
				throws IOException {
			System.out.println("Installing");
			String installPath = Settings.getSettings().getInstallPath();
			LaunchFrame.jarMods = new String[new File(installPath + "/temp/"
					+ modPackName + "/instMods").listFiles().length];

			try {
				FileInputStream fstream = new FileInputStream(installPath
						+ "/temp/" + modPackName + "/modlist");
	...
	private boolean upToDate() throws IOException {
		...
		File version = new File(Settings.getSettings().getInstallPath()
				+ File.separator + pack.getDir() + File.separator + "version");
		if (!version.exists()) {
		...
		if ((line = in.readLine()) == null
				|| Integer.parseInt(pack.getVersion()) > Integer.parseInt(line)) {
			System.out.println("File found, out of date.");
			ModpackUpdateDialog p = new ModpackUpdateDialog(
					LaunchFrame.getInstance(), true);
			p.setVisible(true);
			in.close();
			if (!update) {
				return true;
	...
	public static void cleanUp() {
		ModPack pack = ModPack.getPack(LaunchFrame.getSelectedModIndex());
		File tempFolder = new File(Settings.getSettings().getInstallPath()
				+ File.separator + "temp" + File.separator + pack.getDir()
				+ File.separator);
	...
	private boolean mustBeUninstall(File file, String base,
			ArrayList<Pattern> uninstallList) {
