...
//		packInfo.setBackground(UIManager.getColor("control").darker().darker());
	...
	public static void updateFilter() {
		typeLbl.setText("<html><body><strong>Filter:</strong> " + type + " / " + origin +"</body></html>");
		sortPacks();
	...
	private static boolean isPackVisible(ModPack pack) {
		...
		else if (origin.equalsIgnoreCase("chopnix") && pack.getAuthor().equalsIgnoreCase("chopnix")) {
			return true;
		}
		else if(origin.equalsIgnoreCase("ftb") && pack.getAuthor().equalsIgnoreCase("the ftb team")) {
			return true;
		}
		else if(origin.equalsIgnoreCase("3rd Party") && !pack.getAuthor().equalsIgnoreCase("chopnix")) {
			return true;
		}
