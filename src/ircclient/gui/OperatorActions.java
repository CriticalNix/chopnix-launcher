...
public class OperatorActions extends Thread {
	...
	public void run() {
		...
		try {
			String nick = ul.getList().getElementAt(ul.getSelectedIndex())
					.toString();
			...
			Output o = ul.getChannelPanel().getServPane().getServer()
					.getOutput();
	...
	public void ban(String nick, Output o) throws IOException {
		...
		o.userhost(nick);
		while (!ul.getChannelPanel().getServPane().getServer().getInput()
				.isUserHost()) {
		}
		String host = ul.getChannelPanel().getServPane().getServer().getInput()
				.getUserHost();
		if (host != null) {
			String[] split = host.split("@");
			System.out.println(host);
			o.ban("*!*" + split[0] + "@" + split[1], ul.getChannelPanel()
					.getChannel());
		}
	}
}
