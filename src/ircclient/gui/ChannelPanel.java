...
public class ChannelPanel extends JPanel {
	...
	public ActionListener nickListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			String newnick = JOptionPane.showInputDialog(getThis(),
					"Choose a new nick", "Change nick",
	...
	public ActionListener topicListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				getServPane().getServer().getOutput()
						.topic(getChannel(), newTopic);
			} catch (IOException ioe) {
			}
		}
	};
				...
				try {
					if (!getText.startsWith("/")) {
						getServPane().getServer().getOutput()
								.say(chan, getText);
						getChat().appendLine(
								getServPane().getServer().getNick(), getText);
					} else {
