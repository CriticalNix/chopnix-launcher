...
public class Input {
	...
	public void parse(String in) {
		...
		} else if (split[1].equals("PART") || split[1].equals("QUIT")) { // Handles
																			// joins,parts
																			// and
																			// quits
			parseLeave(in, split);
		...
		} else if (split[1].equals("MODE")) { // Handles all modes for channel
												// and users
			parseMode(in, split);
		...
		} else if (split[1].equals("311")
				|| split[1].equals("307")
				|| split[1].equals("319") // Handles whois message
				|| split[1].equals("312") || split[1].equals("317")
				|| split[1].equals("318")) {
			parseWhois(in, split);
		...
		} else if (split[1].equals("TOPIC")) { // Handles topic when user is in
												// channel
			parseTopic(in, split);

		} else if (split[1].equals("332")) { // Handles topic when user joins
												// channel
			parseStartTopic(in, split);

		} else if (split[1].equals("333")) { // Handles 2nd part of topic when
												// user joins channel
			parseTopicInfo(in, split);

		} else if (split[1].equals("376")) { // Handles actions after end of
												// /MOTD (Autojoin etc)
		// autoJoin();
	...
	private boolean parseUserlist(String in, String[] split) {
		...
		SortedListModel d = serv.getServerPanel().getChannelPane(split[4])
				.getUserlist().getList();
		for (String user : users) {
			d.add(user);
		}
		serv.getServerPanel().getChannelPane(split[4]).getOnlineCount()
				.setText(d.getSize() + " users");
		return true;
	}

	private boolean parseChannelList(String in, String[] split) {
		int offset = split[0].length() + split[1].length() + split[2].length()
				+ split[3].length() + split[4].length() + 6;
	...
	private boolean parseKick(String in, String[] split) {
		...
		serv.getServerPanel().getChannelPane(channel).getChat()
				.appendKick(kicker, kicked, channel, reason);

		serv.getServerPanel().getChannelPane(channel).getUserlist().getList()
				.removeElement(kicked);
		String[] ops = { "@", "&", "%", "+", "~" };
		for (String op : ops) {
			serv.getServerPanel().getChannelPane(channel).getUserlist()
					.getList().removeElement(op + kicked);
		}

		if (kicked.equals(serv.getNick())) {
			serv.getServerPanel().getChannelPane(channel).getTopic()
					.setText("");
			serv.getServerPanel().getChannelPane(channel).getUserlist()
					.getList().clear();
		}
	...
	private boolean parseTopic(String in, String[] split) {
		...
		try {
			...
			serv.getServerPanel().getChannelPane(channel).getTopic()
					.setText(topic);
			serv.getServerPanel()
					.getChannelPane(channel)
					.getChat()
					.appendTopic(
							"*" + name + " has changed the topic to: " + topic);
	...
	private boolean parseStartTopic(String in, String[] split) {
		try {
			String topic = in.split("332 " + serv.getNick() + " " + split[3]
					+ " :")[1];
			String channel = split[3];
			serv.getServerPanel().getChannelPane(channel).getTopic()
					.setText(topic);
			serv.getServerPanel().getChannelPane(channel).getChat()
					.appendTopic("*Topic for " + channel + ": " + topic);
		} catch (Exception e) {
		}
		return true;
	}
	...
	private boolean parseTopicInfo(String in, String[] split) {
		...
		String date = new java.text.SimpleDateFormat("HH:mm:ss dd/MM/yyyy ")
				.format(new java.util.Date(time * 1000));
		serv.getServerPanel()
				.getChannelPane(channel)
				.getChat()
				.appendTopic(
						"*Topic for " + channel + " set by " + user + " at "
								+ date);
		return true;
	}
	...
	private boolean parseLeave(String in, String[] split) {
		...
		for (String op : ops) {
			serv.getServerPanel().getChannelPane(channel).getUserlist()
					.getList().removeElement(op + name);
		}

		if (split[1].equals("PART")) {
			serv.getServerPanel().getChannelPane(channel).getChat()
					.appendPart(name + " (" + host + ")", channel);
		} else {
			for (ChannelPanel cp : serv.getServerPanel().getChannelList()) {
				...
				for (String op : ops) {
					if (cp.getUserlist().getList().contains(op + name)) {
						cp.getChat().appendQuit(name + " (" + host + ")",
								channel);
					}
				}
		...
		/*
		 * try { if (log) { new Log(serv.getServerPanel().getServerName(),
		 * channel).writeLine(name + "(" + host + ") has left " + channel); } }
	...
	private boolean parseNotice(String in, String[] split) {
		...
		try {
			sp.getChannelPane(sp.getTabbedPane().getSelectedIndex()).getChat()
					.appendNotice(name, line);
		} catch (Exception e) {
		}
		return true;
	}
	...
	/*
	 * private boolean autoJoin() { try { if (serv.nickPass() != null) {
	 * serv.getOutput().say("NickServ", "id " + serv.nickPass()); }
	 * 
	 * if (serv.getAutojoin() != null) { for (String chan : serv.getAutojoin())
	 * { serv.getOutput().join(chan); serv.getServerPanel().joinChan(chan); } }
	 * } catch (IOException ioe) { } return true; }
	 */
	private boolean parsePrivmsg(String in, String[] split) { // Handles all
																// msgs.
		String line = getLine(in);
		String name = in.toString().split("!")[0].substring(1);
		...
		if (!line.contains("\u0001")) { // If normal msg
			...
			if (!channel.equals(serv.getNick())) { // If channel message
				if (line.toLowerCase().contains(serv.getNick().toLowerCase())) { // If
																					// highlight
					Tray t = serv.getServerPanel().getWindow().getTray();
					if (t.isTray()
							&& !serv.getServerPanel().getWindow().isFocused()) {
						t.message("Highlight", name + ": \n" + line);
					}
					serv.getServerPanel().getChannelPane(channel).getChat()
							.appendHighlight(name, line);
				...
				serv.getServerPanel().getChannelPane(name).getChat()
						.appendLine(name, line);
			}
		...
		/*
		 * try { if (log) { new Log(serv.getServerPanel().getServerName(),
		 * channel).writeLine("<" + name + "> " + line); } } catch (IOException
		 * ioe) { }
		 */
		return true;
	}
	...
	private boolean parseMode(String in, String[] split) {
		...
		if (split.length == 5) {
			...
			if (mode.equals("+v")) {
				cp.getChat().appendLine(
						"*" + name + " gives voice to " + victim);
				cp.getUserlist().getList().removeElement(victim);
				cp.getUserlist().getList().add("+" + victim);
			} else if (mode.equals("-v")) {
				cp.getChat().appendLine(
						"*" + name + " removes voice from " + victim);
				cp.getUserlist().getList().removeElement("+" + victim);
				cp.getUserlist().getList().add(victim);
			} else if (mode.equals("+o")) {
				cp.getChat().appendLine(
						"*" + name + " gives operator to " + victim);
				cp.getUserlist().getList().removeElement(victim);
				cp.getUserlist().getList().add("@" + victim);
			} else if (mode.equals("-o")) {
				cp.getChat().appendLine(
						"*" + name + " removes operator from " + victim);
				cp.getUserlist().getList().removeElement("@" + victim);
				cp.getUserlist().getList().add(victim);
			} else if (mode.equals("+q")) {
				cp.getChat().appendLine(
						"*" + name + " gives owner to " + victim);
				cp.getUserlist().getList().removeElement(victim);
				cp.getUserlist().getList().add("~" + victim);
			} else if (mode.equals("-q")) {
				cp.getChat().appendLine(
						"*" + name + " removes owner from " + victim);
				cp.getUserlist().getList().removeElement("~" + victim);
				cp.getUserlist().getList().add(victim);
				...
				cp.getChat().appendLine(
						"*" + name + " gives half-operator to " + victim);
				cp.getUserlist().getList().removeElement(victim);
				cp.getUserlist().getList().add("%" + victim);
			} else if (mode.equals("-h")) {
				cp.getChat().appendLine(
						"*" + name + " removes half-operator from " + victim);
				cp.getUserlist().getList().removeElement("%" + victim);
				cp.getUserlist().getList().add(victim);
	...
	private boolean parseCTCP(String in, String[] split) throws IOException {
		...
		if (in.toLowerCase().contains("\u0001version")) {
			...
			int index = serv.getServerPanel().getTabbedPane()
					.getSelectedIndex();

			serv.getServerPanel().getChannelPane(index).getChat()
					.appendLine("*Recieved CTCP VERSION from " + name);
			serv.getOutput().sendNotice(
					name,
					"VERSION " + serv.getServerPanel().getWindow().getTitle()
							+ " [" + os + " " + version + " " + arch + "]");

		} else if (in.toLowerCase().contains("\u0001action")) {
			String[] splitt = in.toLowerCase().split("\u0001action");
			serv.getServerPanel().getChannelPane(split[2]).getChat()
					.appendCTCP(name, splitt[1]);
		} else {
