...
import net.ftb.gui.panes.Online;
...
public class LaunchFrame extends JFrame {
	...
	public LaunchFrame(final int tab) {
		...
		//tabbedPane.setIconAt(3, new ImageIcon(this.getClass().getResource("/image/tabs/texturepacks.png")));
	...
	/**
	 * @param url - the name of the file, as saved to the repo (including extension)
	 * @return - the direct link
	 * @throws NoSuchAlgorithmException - see md5
	 */
	
	public static String getFullLink(String url) throws NoSuchAlgorithmException {
		String resolved;
		...
		if (url.startsWith("bs:")) {
			...
			resolved = "http://repo.creeperhost.net/direct/FTB2/" + currentmd5 + "/" + url.substring(3);
		}
		else if (url.startsWith("sc:")) {
			resolved = "http://repo.creeperhost.net/static/FTB2/"  + url.substring(3);
		}
		else {
			resolved = url;
	...
	protected void installMods(String modPackName) throws IOException {
		...
		FileUtils.copyFolder(source, new File(installpath + "/" + pack.getDir() + "/minecraft/"), false);
	}
